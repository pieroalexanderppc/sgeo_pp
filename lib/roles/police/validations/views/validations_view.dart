...
// Rediseño visual v2: tabs en accent alertAmber, StatusBadge reemplaza el chip de
// estado armado a mano en el tab de Historial.
...
class _ValidationsViewState extends State<ValidationsView>
  ...
  void initState() {
    ...
    _fetchReportes();
    // Escucha reportes enviados desde el mismo dispositivo (mismo proceso).
  ...
  Future<void> _fetchReportes() async {
    ...
    try {
      ...
      final pendientes = reportes
          .where(
            (r) => (r['estado'] ?? '').toString().toLowerCase() == 'pendiente',
          )
          .toList();
      final historial = reportes
          .where(
            (r) => [
              'confirmado',
              'rechazado',
            ].contains((r['estado'] ?? '').toString().toLowerCase()),
          )
          .toList();
        ...
        for (var group in grouped) {
          if (group['subtipo_hecho'] == report['subtipo_hecho']) {
            try {
              ...
              if (distance <= 500) {
                group['cantidad_agrupada'] =
                    (group['cantidad_agrupada'] ?? 1) + 1;
  ...
  Future<void> _updateStatus(String reportId, {required bool confirmar}) async {
    ...
    final confirm = await showDialog<bool>(
      context: context,
      // Sin backgroundColor/shape propios: hereda dialogTheme (bgElevated/borderSubtle).
      builder: (ctx) => AlertDialog(
        title: Row(
          children: [
            Icon(
              confirmar ? Icons.check_circle_outline : Icons.cancel_outlined,
              color: accentColor,
              size: 28,
            ),
        ...
        content: Text(
          message,
          style: TextStyle(
            color: isDark ? AppTheme.textSecondary : Colors.grey[800],
          ),
        ),
        actions: [
          TextButton(
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                rep['direccion_hecho'] ?? 'GPS desconocido',
                                style: TextStyle(
                              ...
                              const SizedBox(height: 4),
                              Text(
                                _formatFecha(rep['fecha_hora_hecho']),
                                style: TextStyle(
                                  fontSize: 12,
    ...
    return SafetyLayout(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          indicatorColor: AppTheme.alertAmber,
          labelColor: AppTheme.alertAmber,
  ...
  Widget _buildHistorialTab(bool isDark) {
    return RefreshIndicator(
      ...
      child: _historial.isEmpty
          ? _buildEmptyState(
              isDark,
              'Aún no hay reportes confirmados o rechazados',
            )
          : ListView.builder(
              padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 14),
              ...
              itemBuilder: (ctx, i) {
                ...
                final estadoColor = isConfirmado
                    ? AppTheme.successGreen
                    : AppTheme.alertRed;
                final fecha = isConfirmado
                    ? rep['confirmado_en']
                    : rep['rechazado_en'];
