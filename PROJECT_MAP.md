...
| @roles_police | lib/roles/police/{home,map,profile,validations}/views/ | Shell policial, mapa táctico, panel de validación, perfil |
...
| Alias | Ruta | Responsabilidad |
|-------|------|-----------------|
| @routes | backend/routes/ | Endpoints: `auth.py`, `reports.py`, `maps.py`, `predictive.py`, `users.py`, `admin.py` |
| @models_be | backend/models/ | Esquemas Pydantic: `auth_schemas.py`, `user_schemas.py`, `report_schemas.py` |
| @services_be | backend/services/ | `report_service.py` (validación/confirmación de denuncias), `analytics_service.py` (regresión lineal) |
...
| POST | `/api/auth/login` | routes/auth.py → `login` | Autenticación email/password (bcrypt) |
| POST | `/api/auth/register` | routes/auth.py → `register` | Registro de usuario (rol: ciudadano/policia/admin) |
...
| GET | `/api/reportes/policia` | routes/reports.py → `obtener_reportes_policia` | Lista reportes pendientes/confirmados para policía |
...
| Mapa táctico (policía) | @map_police | @routes (maps.py) | ✅ completo |
| Reportes ciudadanos (CRUD) | @report_dialog, lib/roles/user/reports/ | @routes (reports.py) + @services_be | ✅ completo |
| Validación policial de reportes | lib/roles/police/validations/ | @routes (reports.py: confirmar/rechazar) | ✅ completo |
| Dashboard admin (live + Big Data SIDPOL) | lib/roles/admin/dashboard/ | @routes (admin.py) + @services_be (analytics_service.py) | ✅ completo |
| Gestión de usuarios (admin) | lib/roles/admin/users/ | @routes (users.py) | ⚠️ parcial — backend solo expone GET/PUT por id, no se confirmó endpoint de suspensión/borrado de cuentas que menciona el README `[REVISAR]` |
...
- **Estado / State management:** sin librería externa — `setState` local + `ValueNotifier` para eventos cross-widget (`ReportService.reportsUpdatedNotifier`, `AppTheme.themeNotifier`, `NotificationsStorageService.updateNotifier`, `TutorialService.triggerTutorialNotifier`)
